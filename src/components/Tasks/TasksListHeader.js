	...
	if (props.board.editing) {
		taskHeaderContent = (
			<form
				method="POST"
				onSubmit={event =>
					props.submitBoardNameChange(event, props.board.id)
				}
			>
				<input
					...
					id="board-name"
					onChange={event =>
						props.onChangeBoardName(event, props.board.id)
					}
					onBlur={event =>
						props.submitBoardNameChange(event, props.board.id)
					}
	...
	return (
		<div className="tasks-box-header">
			<span
				...
				onClick={() => {
					props.boardNameChange(props.board.id);
				}}
			>
				{taskHeaderContent}
			</span>
